...
export default function AdminSidebar({ user, onLogout, router }: AdminSidebarProps) {
  return (
    <>
      {/* Sidebar untuk versi desktop, disesuaikan dengan ukuran layar medium berubah jadi display flex
       display flex berarti sebuah kontainer fleksibel yang dapat mengatur tata letak elemen-elemen di dalamnya (disebut flex items) secara horizontal atau vertikal secara lebih mudah dan responsif.*/}
      <div className="hidden h-screen w-64 flex-shrink-0 flex-col border-r bg-muted/40 p-6 md:flex">
        <div className="flex h-14 items-center border-b">
          <h2 className="text-lg font-semibold">Admin Dashboard</h2>
        </div>
        {/* menampilkan nama dan email dari setiap admin/super admin didalam dashboard*/}
        ...
        <nav className="flex-1 pt-4">
          <div className="space-y-1">
            ...
            <Button variant="ghost" className="w-full justify-start" onClick={() => router.push("/admin/destinations-category")}>
              {" "}
              <MapPin className="mr-2 h-4 w-4" />
              <span>Kategori Wisata</span>
            </Button>
            <Button variant="ghost" className="w-full justify-start" onClick={() => router.push("/admin/destinations")}>
              {" "}
              <MapPin className="mr-2 h-4 w-4" />
              <span>Destinasi Wisata</span>
            </Button>
            <Button variant="ghost" className="w-full justify-start" onClick={() => router.push("/admin/tickets")}>
              {" "}
              <Ticket className="mr-2 h-4 w-4" />
              <span>Transaksi E-Ticket</span>
            </Button>
            <Button variant="ghost" className="w-full justify-start" onClick={() => router.push("/admin/orders")}>
              {" "}
              <ShoppingCart className="mr-2 h-4 w-4" />
              <span>Daftar Pengunjung</span>
            </Button>
            <Button variant="ghost" className="w-full justify-start" onClick={() => router.push("/admin/complaints")}>
              {" "}
